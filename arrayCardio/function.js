      ...
      const birthsort = inventors.sort((i1, i2) => i1.year > i2.year);
      console.table(birthsort);

      // Array.prototype.reduce()
      // 4. How many years did all the inventors live all together?
      const togetherlive = inventors.reduce((total, curr) => {
        return total + curr.passed - curr.year;
      }, 0);
      console.log(togetherlive);
      ...
      const transportation = data.reduce((res, item) => {
        if (!res[item]) {
          res[item] = 0;
        }
        res[item]++;
